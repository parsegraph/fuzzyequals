...
import fuzzyEquals, {cloneFuzziness, getFuzziness, setFuzziness} from "../dist/fuzzyequals";
...
describe("fuzzyEquals", function () {
  ...
  it("can be changed globally", ()=>{
    ...
    assert.equal(fuzzyEquals(1, 1.1), true);
    setFuzziness(.0001);
    assert.equal(getFuzziness(), .0001);
...
describe("Fuzziness", function () {
  ...
  it("is subject to default fuzziness", ()=>{
    setFuzziness(.0001);
    const fuzziness = cloneFuzziness();
    assert.equal(fuzziness.getFuzziness(), .0001, "cloned fuzziness initially uses the default fuzziness");
    fuzziness.resetFuzziness();

    setFuzziness(1);
    assert.equal(fuzziness.getFuzziness(), 1, "Reset fuzziness will default to the current default fuzziness");

    assert.equal(fuzziness.check(1, 1.1), true, "Large fuzziness should treat close values as equal");
  });
